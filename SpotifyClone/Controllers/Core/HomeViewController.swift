...
class HomeViewController: UIViewController {
    ...
    @objc func didLongPress(_ gesture: UILongPressGestureRecognizer) {
        ...
        //Action Sheet
        let actionSheet = UIAlertController(title: model.name, message: "Çalma listenize eklemek ister misiniz?", preferredStyle: .actionSheet)
        actionSheet.addAction(UIAlertAction(title: "Vazgeç", style: .cancel))
        actionSheet.addAction(UIAlertAction(title: "Çalma Listesine Ekle", style: .default, handler: { [weak self] _ in
            let vc = LibraryPlaylistViewController()
            vc.selectionHandler =  { playlist in
                APIManager.shared.addTrackToPlaylist(
                    track: model,
                    playlist: playlist) { succes in
                     print("Çalma listesine eklendi: \(succes)")
                }
            }
...
extension HomeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        switch type {
        case .newReleases:
            header.configure(with: "Yeni Çıkanlar")
        case .featuredPlaylists:
            header.configure(with: "Öne Çıkan Çalma Listeleri")
        case .recommendedTracks:
            header.configure(with: "Tavsiye Parçalar")
        }
        return header
    }
